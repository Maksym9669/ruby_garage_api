...
module Api
  class PriorityDownController < ApplicationController
    ...
    def update
      authorize task
      result = task_mover.move_down
      case result
      when Constants::TASK_MOVED
        render json: { success: { text: I18n.t("Task was moved down") } }, status: 200
      when Constants::TASK_CANT_MOVE
        render json: { "error": I18n.t("This task can't be moved down") }, status: 400
